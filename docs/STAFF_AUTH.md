> Pending username-login implementation supersedes the phone-login instructions below. See [USERNAME-LOGIN.md](USERNAME-LOGIN.md) for the current local flow and rollout prerequisites.
...
3. In Supabase **Authentication → Sign In / Providers**, disable **Allow new users to sign up** and anonymous sign-ins. Keep email/password available and enable Phone authentication. Production was checked read-only on 2026-09-16: minimum password length is 6, with no required character classes. The shared application policy and local config match this setting. Recheck hosted settings before changing that policy. Configure SMS directly in Supabase if using OTP verification; the app's admin flow requires the administrator to verify phone ownership before confirming the number. Local `supabase/config.toml` does not change hosted settings.
...
For a reset, choose the existing user and **Reset password**. Enter a new temporary password manually. The reset blocks access during the operation, restores the first-login gate and invalidates earlier application JWTs using a server timestamp cutoff. Auth password changes also revoke refresh sessions. A login attempted within the first second after reset may require retrying once.
...
Creation requires a manually chosen temporary password; resets also require a manually entered temporary password. Chosen passwords require at least 6 characters server-side before any account operation starts, with no application character-complexity requirements. Confirmation must match exactly. All password inputs and the one-time temporary password display start hidden and support localized Show/Hide controls. The value is sent only to Supabase Auth, never to application SQL RPC payloads. Inputs clear after submission and on page hiding; the one-time response is also hidden when the page loses visibility. No password retrieval route exists. See STAFF-PASSWORD-ACCEPTANCE.md for real-device acceptance steps.
