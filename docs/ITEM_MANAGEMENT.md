...
Owners can use **Inventory → Add item** to create a product and its location configuration in one transaction. Categories come from the database; units use the existing shared stock-unit enum. New items start at zero. Inactive items remain hidden from operational lists. Category administration stays owner-only.
...
Excel actions and the import screen are shown only on desktop/tablet layouts. Managers cannot call the catalog/import RPC or access the workbook endpoint. See SIMPLIFIED_OPERATIONS.md for the current operational roles and receipt flow.
