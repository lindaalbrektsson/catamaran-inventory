...
The pending local Need photo enhancement is documented in [NEED-PHOTOS-2026-09-22.md](NEED-PHOTOS-2026-09-22.md). OWNER/MANAGER can select or capture JPEG/PNG/WebP sources up to 30 MiB, optimized locally to JPEG at most 3 MiB, using the shared media picker. Photos use the private `need-photos` bucket and trusted server-only finalization after actual byte validation. Replacement creates an immutable version; existing legacy photos remain readable. Need metadata remains if upload fails; retry the unchanged request or reopen the saved entry. Reference-photo originals are not preserved. Reading requires authenticated OWNER/MANAGER access and is never cached. This pending change does not alter stock.
