-- Controlled catalog writes for operational staff. Existing RLS is unchanged.
create table private.item_batches (
  id uuid primary key, actor uuid not null references public.profiles(id),
  payload jsonb not null, created_at timestamptz not null default now()
);
revoke all on private.item_batches from public,anon,authenticated;

create function public.save_inventory_items(p_id uuid,p_rows jsonb) returns uuid
...
begin
  if auth.uid() is null or (private.current_role() in ('OWNER','MANAGER')) is not true then raise exception 'FORBIDDEN'; end if;
