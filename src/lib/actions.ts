...
import { z } from 'zod';
export type ActionState={error?:Key;fields?:Partial<Record<'quantity'|'reason'|'notes',Key>>};
...
export async function signIn(_previous:ActionState,form:FormData):Promise<ActionState> {
  ...
  const result=z.object({email:z.email().max(254),password:z.string().min(1).max(1024)}).safeParse(Object.fromEntries(form));
  ...
  const {data:profile}=await db.from('profiles').select('language').eq('id',data.user.id).maybeSingle();
...
export async function changeStock(_previous:ActionState,form:FormData):Promise<ActionState> {
  ...
  const permission=value.mode==='add'?'inventory.add':type==='TOUR_USE'?'inventory.consume':'inventory.remove';
  ...
  const {error}=await db.rpc('change_stock',{
    p_request_id:value.requestId,p_product_id:value.productId,p_location_id:value.locationId,
    p_quantity:Number(value.quantity),p_type:type,p_reason:value.reason,p_notes:value.notes,
  });
  if(error) {
    const known=['INSUFFICIENT_STOCK','FORBIDDEN','NOT_FOUND','INVALID_INPUT','REQUEST_CONFLICT'] as const;
